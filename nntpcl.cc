//  $Id: nntpcl.cc 1.28 1997/04/20 19:18:48 hardy Exp $
//
...
//
{
	...
	if (killEnabled  &&  killQHook != NULL) {
	    long oldScore = articleScore;
	    articleScore += killQHook(selGroup,bufp);
	    if ((oldScore >= killThreshold  &&  articleScore < killThreshold)  ||
		(!killReasonFound  &&  strnicmp(bufp,"subject:",8) == 0)) {
